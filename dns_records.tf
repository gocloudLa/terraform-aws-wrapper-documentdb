...
/*----------------------------------------------------------------------*/
data "aws_route53_zone" "documentdb" {
  for_each = local.documentdb_route53
...
resource "aws_route53_record" "documentdb" {
  ...
  zone_id         = data.aws_route53_zone.documentdb[each.key].zone_id
  name            = lookup(each.value, "record_name")
  allow_overwrite = false
  type            = "CNAME"
  ttl             = lookup(each.value, "ttl")
  records         = [module.documentdb[each.value.name].endpoint]
}

locals {
  documentdb_route53_tmp = [for resource_name, value1 in var.documentdb_parameters :
    {
      for dns_record_name, value2 in value1.dns_records :
      "${resource_name}-${dns_record_name}" =>
      {
        "name"         = resource_name
        "record_name"  = length(dns_record_name) > 0 ? dns_record_name : "${local.common_name}-${resource_name}.documentdb"
  ...
  documentdb_route53 = merge(local.documentdb_route53_tmp...)
}